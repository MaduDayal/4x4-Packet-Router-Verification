class direct_smoke_test extends base_test;
  ...
  task directSmokeTest(input logic [31:0] headerBeat, input logic [31:0] payload[]);
    wait(rEnv.inMonitor.inM.inMonitor.RESET == 1'b0);
    rEnv.run();
    rEnv.packGen.dirPacks(headerBeat, payload);
    
    wait(rEnv.rScoreBoard.num_expected == 1 && rEnv.rScoreBoard.num_actual == 1 && rEnv.packCov.num_sampled == 1);
